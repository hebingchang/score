...
{
	if ($str == $row[0]) {
			header("Location: signup.php?info=userexist"); 
			@mysql_close($dbh); 
			exit;
	}
}
	$sql = 'INSERT INTO user VALUE("' . $str . '", "' . $strenc . '")';
	mysql_query($sql,$dbh);
	$sql = 'INSERT INTO score VALUE("' . $str . '", 0, "' . date("Y-m-d",time()) . '")';
	mysql_query($sql,$dbh);
	header("Location: login.php?info=signupsuccess"); 
